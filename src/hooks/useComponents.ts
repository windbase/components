...
export function useComponents() {
  ...
  const updateComponent = async (originalId: string, componentData: Component, nameChanged: boolean = false) => {
    try {
      ...
      if (result.success) {
        if (nameChanged) {
          ...
          setComponents(prev => prev.filter(comp => comp.metadata.id !== originalId));
          setComponents(prev => [...prev, componentData]);
        } else {
