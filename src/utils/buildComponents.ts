...
export function buildComponents() {
      ...
      components.forEach(componentName => {
        ...
        if (existsSync(metadataPath) && existsSync(htmlPath)) {
          try {
            ...
            writeFileSync(
              join(distPath, type, `${metadata.id}.html`),
              html
            );
  ...
  console.log(`📄 blocks.json (${Object.keys(output.blocks).length} components)`);
  console.log(`📄 templates.json (${Object.keys(output.templates).length} components)`);
